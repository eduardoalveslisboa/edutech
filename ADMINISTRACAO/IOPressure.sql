...
-- Quais queries estão fazendo maior IO (Pode levar alguns segundos)--30/07/2015
...
-- Sempre procura baseado em  Avg Disk Sec/Read and Avg Disk Sec/Write 
-- No PerfMon para cada Physical Disk 


-- Isola TOP Waits por servidor(Instancia) desde o ultimo restart do servidor ou limpeza das estatisticas
-- Olha por  I/O especifico ( Waitsno topo da lista)
WITH Waits AS
...
-- Ajuda a determinar quais arquivos de bancos tem maior gargalo na instancia.
...
-- Especfico para database
USE SeuDatabase;
GO

-- Top Cached SPs pelo total de leitura fisica(SQL 2008). 
-- Leitura fisica relacionada à pressão de IO
...
-- Top Cached SPs pelo total de escrita fisica(SQL 2008). 
-- Escrita fisica relacionada à pressão de IO
...
-- Ajuda a achar a query que mais consome com IO.
