...
USE MONITORSQL
GO
CREATE PROCEDURE DBASP_CONTADORES
AS
...
FROM @RatioStatsX WHERE counter_name = 'Page Splits/sec'
...
INSERT INTO MONITORSQL.DBO.CONTADORES
...
on a.x = s.x
join
(SELECT (cntr_value - @LockWaitsPerSecond) /(CASE WHEN datediff(ss,@stat_date, getdate()) = 0
THEN  1 ELSE datediff(ss,@stat_date, getdate()) END) AS [LockWaitsPerSecond],1 x FROM @RatioStatsY
WHERE counter_name = 'Lock Waits/sec'
AND instance_name = '_Total' AND object_name LIKE '%Locks%') AA
on a.x=AA.x
JOIN
(SELECT (cntr_value - @PageSplitsPerSecond) /(CASE WHEN datediff(ss,@stat_date, getdate()) = 0
THEN  1 ELSE datediff(ss,@stat_date, getdate()) END) AS [PageSplitsPerSecond],1 x
FROM @RatioStatsY WHERE counter_name = 'Page Splits/sec' AND object_name LIKE '%Access Methods%') AB
ON a.x=AB.x